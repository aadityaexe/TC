...
export default function LoginPage() {
  ...
  // const inFlight = useRef(false)
  const { cc, phone } = useSyncExternalStore(
    subscribe,
    getSnapshot,
    getSnapshot
  );
  ...
  const handleNext = async (e) => {
    ...
    try {
      ...
      const full = `${ccNorm.startsWith("+") ? ccNorm : `+${ccNorm}`}${String(
        phoneNow
      ).replace(/\s+/g, "")}`;
      ...
      router.push(
        `/login?phone=${encodeURIComponent(full)}&hash=${encodeURIComponent(
          phoneCodeHash
        )}`
      );
    } catch (err) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-white p-4">
      ...
      <div>
        <img
          ...
          src="./tele-logo.svg"
          alt="Telegram Logo"
          className="w-[10rem] h-[10rem] mb-[2.5rem]"
        />
      </div>
